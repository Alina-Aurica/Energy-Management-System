...
public class JwtService {
    ...
    public String generateToken(UserDetails userDetails){
        Optional<Users> userOptional = userRepository.findByEmail(userDetails.getUsername());
        Map<String, Object> claims = new HashMap<>();
        if(userOptional.isPresent()){
            claims.put("clientID", userOptional.get().getId());
            claims.put("email", userOptional.get().getEmail());
            claims.put("role", userOptional.get().getRole());
        }
